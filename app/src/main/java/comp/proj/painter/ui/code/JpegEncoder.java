package comp.proj.painter.ui.code;// Version 1.0a
...
public class JpegEncoder {
    ...
    public JpegEncoder(Bitmap image, int quality, OutputStream out, String text) {
...
//        }
        ...
        dct2 = new DCT2(Quality);
        Huf = new Huffman(imageWidth, imageHeight);
        Embed = new Embed(text);
        //System.out.println(Embed.getCount());
    }
...
//                        if ((!JpegObj.lastColumnIsDummy[comp] || c < Width - 1) && (!JpegObj.lastRowIsDummy[comp] || r < Height - 1)) {
                            dctArray2 = dct2.forwardDCT(dctArray1);
                            dctArray3 = dct2.quantizeBlock(dctArray2, JpegObj.QtableNumber[comp]);

                            Embed.read(dctArray3);
                        }
                    }
                }
            }
            ...
            for (c = 0; c < MinBlockWidth; c++) {
                ...
                for (comp = 0; comp < JpegObj.NumberOfComponents; comp++) {
                    ...
                    for (i = 0; i < JpegObj.VsampFactor[comp]; i++) {
                        ...
                        for (j = 0; j < JpegObj.HsampFactor[comp]; j++) {
                           int[][] afterEmbed = Embed.listToArray();
                            //System.out.println(Arrays.toString(afterEmbed[count]));
                            Huf.HuffmanBlockEncoder(outStream, afterEmbed[count], lastDCvalue[comp], JpegObj.DCtableNumber[comp], JpegObj.ACtableNumber[comp]);
                            lastDCvalue[comp] = afterEmbed[count][0];
                            count++;
                        }
                    }
                }
        ...
        //System.out.println(Embed.imgArray.size());
...
//                        outputData[index] = (int)(((inputData[i][j] * (((double[]) (Divisors[code]))[index])) + 16384.5) -16384);
    ...
    public int[] getQuantization(double inputData[][], int code) {
        return quantizeBlock(inputData, code);
    }
...
// the only choice.
//        imageobj.getPixels(imageobj.getSource(), 0,0,0, 0,imageWidth, imageHeight);
                //= new PixelGrabber(imageobj.getSource(), 0, 0, imageWidth, imageHeight, values, 0, imageWidth);

        for (index = 0, y = 0; y < imageHeight; ++y) {
            for (x = 0; x < imageWidth; ++x) {
                values[index] = imageobj.getPixel(x, y);
                index++;
            }
        }
        MaxHsampFactor = 1;
